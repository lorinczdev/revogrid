 ...
 * Built by Revolist
 */
import { h, r as registerInstance, c as createEvent, H as Host } from './index-7fd83173.js';
import { i as isFilterBtn, A as AndOrButton, T as TrashButton } from './filter.button-143a6a60.js';
...
const FilterPanel = class {
  ...
  renderExtra(prop, index) {
    ...
    if (this.filterEntities[currentFilter[index].type].extra !== 'input')
      return '';
    return (h("input", { id: `filter-input-${currentFilter[index].id}`,
      // placeholder="Enter value..."
      type: "text", value: currentFilter[index].value, onInput: this.onUserInput.bind(this, index, prop), onKeyDown: e => this.onKeyDown(e) }));
  }
  getFilterItemsList() {
    var _a;
    ...
    return (h("div", { key: this.filterId }, propFilters.map((d, index) => {
      ...
      if (index !== this.filterItems[prop].length - 1) {
        (h("div", { onClick: () => this.toggleFilterAndOr(d.id) }, h(AndOrButton, { isAnd: d.relation === 'and' })));
      }
      return (h("div", { key: d.id, class: FILTER_LIST_CLASS }, h("div", { class: { 'select-input': true } }, h("select", { class: "select-css select-filter", onChange: e => this.onFilterTypeChange(e, prop, index) }, this.renderSelectOptions(this.filterItems[prop][index].type, false)), h("div", { onClick: () => this.onReset() }, h(TrashButton, null))), h("div", null, this.renderExtra(prop, index))));
    })));
  }
  render() {
    if (!this.changes) {
    ...
    return (h(Host, { style: style }, h("label", null, capts.title), h("div", { class: "filter-holder" }, this.getFilterItemsList()), h("div", { class: "filter-actions" }, this.disableDynamicFiltering &&
