...
const localScrollService = require('./localScrollService-948e38e3.js');
...
const RowRenderer = ({ rowClass, index: index$2, size, start, depth }, cells) => {
  const props = Object.assign({ [index.DATA_ROW]: index$2 });
  return (index$1.h("div", Object.assign({}, props, { class: `rgRow ${rowClass || ''}`, style: {
      height: `${size}px`,
...
const RevogridCellRenderer = class {
  constructor(hostRef) {
    ...
    this.beforeCellRender = index$1.createEvent(this, "before-cell-render", 7);
    this.dragStartCell = index$1.createEvent(this, "dragStartCell", 7);
  ...
  render() {
    ...
    const defaultProps = {
      [index.DATA_COL]: columnProps.itemIndex,
      [index.DATA_ROW]: rowProps.itemIndex,
      style: {
    ...
    // if custom render
    if (tpl) {
      return (index$1.h(index$1.Host, Object.assign({}, props), tpl(index$1.h, Object.assign(Object.assign({}, model), { providers: this.providers }), this.additionalData)));
    }
    ...
    if (model.column.rowDrag && isRowDragService(model.column.rowDrag, model)) {
      els.push(index$1.h("span", { class: index.DRAGGABLE_CLASS, onMouseDown: originalEvent => this.dragStartCell.emit({
          originalEvent,
          model,
        }) }, index$1.h("span", { class: index.DRAG_ICON_CLASS })));
    }
    els.push(`${localScrollService.ColumnService.getData(model.model[model.prop])}`);
    // if regular render
    return index$1.h(index$1.Host, Object.assign({}, props), els);
  }
};
...
function expandEvent(e, model, virtualIndex) {
  const event = new CustomEvent(viewport_helpers.GROUP_EXPAND_EVENT, {
    detail: {
      model,
      virtualIndex,
    },
...
const GroupingRowRenderer = (props) => {
  ...
  const name = model[viewport_helpers.PSEUDO_GROUP_ITEM];
  const expanded = model[viewport_helpers.GROUP_EXPANDED];
  const depth = parseInt(model[viewport_helpers.GROUP_DEPTH], 10) || 0;
  if (!hasExpand) {
    return index$1.h(RowRenderer, Object.assign({}, props, { rowClass: "groupingRow", depth: depth }));
  }
  if (groupingCustomRenderer) {
    return (index$1.h(RowRenderer, Object.assign({}, props, { rowClass: "groupingRow", depth: depth }),
      index$1.h("div", { onClick: e => expandEvent(e, model, itemIndex) }, groupingCustomRenderer(index$1.h, { name, itemIndex, expanded, depth }))));
  }
  return (index$1.h(RowRenderer, Object.assign({}, props, { rowClass: "groupingRow", depth: depth }),
    index$1.h("button", { class: { [viewport_helpers.GROUP_EXPAND_BTN]: true }, onClick: e => expandEvent(e, model, itemIndex) },
      index$1.h("svg", { "aria-hidden": "true", style: { transform: `rotate(${!expanded ? -90 : 0}deg)` }, focusable: "false", viewBox: "0 0 448 512" },
        index$1.h("path", { fill: "currentColor", d: "M207.029 381.476L12.686 187.132c-9.373-9.373-9.373-24.569 0-33.941l22.667-22.667c9.357-9.357 24.522-9.375 33.901-.04L224 284.505l154.745-154.021c9.379-9.335 24.544-9.317 33.901.04l22.667 22.667c9.373 9.373 9.373 24.569 0 33.941L240.971 381.476c-9.373 9.372-24.569 9.372-33.942 0z" }))),
    name));
};
...
const RevogrData = class {
  constructor(hostRef) {
    index$1.registerInstance(this, hostRef);
    this.beforeRowRender = index$1.createEvent(this, "beforeRowRender", 7);
    this.afterrender = index$1.createEvent(this, "afterrender", 7);
  ...
  onStoreChange() {
    ...
    this.columnService = new localScrollService.ColumnService(this.dataStore, this.colData);
      ...
      if (this.currentRange) {
        this.renderedRows.forEach((row, y) => {
          ...
          if (row &&
            ...
            row.$elm$.classList.contains(index.ROW_FOCUSED_CLASS)) {
            row.$elm$.classList.remove(index.ROW_FOCUSED_CLASS);
          }
        });
      }
      // apply new range
      if (e) {
        for (let y = e.y; y <= e.y1; y++) {
          ...
          if (row &&
            row.$elm$ instanceof HTMLElement &&
            !row.$elm$.classList.contains(index.ROW_FOCUSED_CLASS)) {
            row.$elm$.classList.add(index.ROW_FOCUSED_CLASS);
          }
        }
      }
      this.currentRange = e;
  ...
  render() {
    ...
    for (let rgRow of rows) {
      const dataItem = index.getSourceItem(this.dataStore, rgRow.itemIndex);
      /** grouping */
      if (viewport_helpers.isGrouping(dataItem)) {
        rowsEls.push(index$1.h(GroupingRowRenderer, Object.assign({}, rgRow, { index: rgRow.itemIndex, model: dataItem, groupingCustomRenderer: groupingCustomRenderer, hasExpand: this.columnService.hasGrouping })));
        continue;
      }
      /** grouping end */
      const cells = [];
      ...
      if (this.currentRange &&
        ...
        rowClass += ` ${index.ROW_FOCUSED_CLASS}`;
      }
      for (let rgCol of cols) {
        cells.push(index$1.h("revogr-cell", { additionalData: this.additionalData, columnService: this.columnService, providers: this.providers, depth: this.columnService.hasGrouping ? depth : 0, rowIndex: rgRow.itemIndex, rowStart: rgRow.start, rowEnd: rgRow.end, rowSize: rgRow.size, colIndex: rgCol.itemIndex, colStart: rgCol.start, colEnd: rgCol.end, colSize: rgCol.size }));
      }
      const row = (index$1.h(RowRenderer, { index: rgRow.itemIndex, rowClass: rowClass, size: rgRow.size, start: rgRow.start }, cells));
    ...
    return (index$1.h(index$1.Host, null, index$1.h("slot", null), rowsEls));
  }
...
var defineProperty = (function() {
  try {
    var func = index._getNative(Object, 'defineProperty');
    func({}, '', {});
    return func;
  } catch (e) {}
}());
...
function createAggregator(setter, initializer) {
  return function(collection, iteratee) {
    var func = index.isArray_1(collection) ? _arrayAggregator : _baseAggregator,
        accumulator = initializer ? initializer() : {};

    return func(collection, setter, index._baseIteratee(iteratee), accumulator);
  };
}
...
const SortingSign = ({ column }) => {
  var _a;
  return index$1.h("i", { class: (_a = column === null || column === void 0 ? void 0 : column.order) !== null && _a !== void 0 ? _a : 'sort-off' });
};
...
const ResizableElement = (props, children) => {
  ...
  if (props.canResize) {
    if (props.active) {
      for (let p in props.active) {
        resizeEls.push(index$1.h("div", { onClick: e => e.preventDefault(), onDblClick: e => {
            var _a;
            e.preventDefault();
    ...
    if (props.active) {
      for (let p in props.active) {
        resizeEls.push(index$1.h("div", { onClick: e => e.preventDefault(), onTouchStart: (e) => e.preventDefault(), onDblClick: e => {
            var _a;
            e.preventDefault();
            (_a = props.onDoubleClick) === null || _a === void 0 ? void 0 : _a.call(props, e);
  ...
  return (index$1.h("div", Object.assign({}, props, { ref: (e) => directive === null || directive === void 0 ? void 0 : directive.set(e) }),
    children,
    resizeEls));
};
...
const HeaderCellRenderer = ({ data, props, additionalData }, children) => {
  ...
  if (data === null || data === void 0 ? void 0 : data.columnTemplate) {
    colTemplate = data.columnTemplate(index$1.h, data, additionalData);
  }
  if (data === null || data === void 0 ? void 0 : data.columnProperties) {
    const extra = data.columnProperties(data);
    if (extra && typeof extra === 'object') {
      cellProps = localScrollService.ColumnService.doMerge(props, extra);
    }
  }
  return (index$1.h(ResizableElement, Object.assign({}, cellProps, { onMouseDown: (e) => {
      dispatcher.dispatch(e.currentTarget, ON_COLUMN_CLICK, {
        data,
        event: e,
      });
    } }),
    index$1.h("div", { class: "header-content" }, colTemplate),
    children));
};
...
const HeaderRenderer = (p) => {
  ...
  const cellClass = {
    [index.HEADER_CLASS]: true,
    [index.HEADER_SORTABLE_CLASS]: !!((_a = p.data) === null || _a === void 0 ? void 0 : _a.sortable),
  };
  if ((_b = p.data) === null || _b === void 0 ? void 0 : _b.order) {
    cellClass[p.data.order] = true;
  }
  const dataProps = {
    [index.DATA_COL]: p.column.itemIndex,
    canResize: p.canResize,
    minWidth: ((_c = p.data) === null || _c === void 0 ? void 0 : _c.minSize) || index.MIN_COL_SIZE,
  ...
  if (p.range) {
    if (p.column.itemIndex >= p.range.x && p.column.itemIndex <= p.range.x1) {
      if (typeof dataProps.class === 'object') {
        dataProps.class[index.FOCUS_CLASS] = true;
      }
    }
  }
  return (index$1.h(HeaderCellRenderer, { data: p.data, props: dataProps, additionalData: p.additionalData },
    ((_e = p.data) === null || _e === void 0 ? void 0 : _e.order) ? index$1.h(SortingSign, { column: p.data }) : '',
    p.canFilter && ((_f = p.data) === null || _f === void 0 ? void 0 : _f.filter) !== false ? index$1.h(filter_button.FilterButton, { column: p.data }) : ''));
};

const GroupHeaderRenderer = (p) => {
  const groupProps = {
    canResize: p.canResize,
    minWidth: p.group.ids.length * index.MIN_COL_SIZE,
    maxWidth: 0,
    active: p.active || ['r'],
    class: {
      [index.HEADER_CLASS]: true,
  ...
  return (index$1.h(HeaderCellRenderer, { data: Object.assign(Object.assign({}, p.group), { prop: '', providers: p.providers, index: p.start }), props: groupProps, additionalData: p.additionalData }));
};
  ...
  for (let i = 0; i < depth; i++) {
    if (groups[i]) {
      for (let group of groups[i]) {
        ...
        const indexFirstVisibleCol = index.findIndex_1(group.ids, id => typeof visibleProps[id] === 'number');
        if (indexFirstVisibleCol > -1) {
          ...
          // coordinates
          const groupStart = index.getItemByIndex(dimensionCol, groupStartIndex).start;
          const groupEnd = index.getItemByIndex(dimensionCol, groupEndIndex).end;
          groupRow.push(index$1.h(GroupHeaderRenderer, { providers: providers, start: groupStart, end: groupEnd, group: group, active: active, canResize: canResize, onResize: e => onResize(e.changedX, groupStartIndex, groupEndIndex), additionalData: additionalData }));
        }
      }
    }
    groupRow.push(index$1.h("div", { class: `${index.HEADER_ROW_CLASS} group` }));
  }
  return groupRow;
};
...
const RevogrHeaderComponent = class {
  constructor(hostRef) {
    index$1.registerInstance(this, hostRef);
    this.initialHeaderClick = index$1.createEvent(this, "initialHeaderClick", 7);
    this.headerresize = index$1.createEvent(this, "headerresize", 7);
    this.beforeResize = index$1.createEvent(this, "before-resize", 7);
    this.headerdblClick = index$1.createEvent(this, "headerdblClick", 7);
    this.viewportCol = undefined;
  ...
  render() {
    ...
    for (let rgCol of cols) {
      ...
      cells.push(index$1.h(HeaderRenderer, { range: range, column: rgCol, data: Object.assign(Object.assign({}, colData), { index: rgCol.itemIndex, providers: this.providers }), canFilter: !!this.columnFilter, canResize: this.canResize, active: this.resizeHandler, onResize: e => this.onResize(e, rgCol.itemIndex), onDoubleClick: e => this.headerdblClick.emit(e), onClick: e => this.initialHeaderClick.emit(e), additionalData: this.additionalData }));
      visibleProps[colData === null || colData === void 0 ? void 0 : colData.prop] = rgCol.itemIndex;
    }
    return [
      index$1.h("div", { class: "group-rgRow" }, index$1.h(ColumnGroupsRenderer, { canResize: this.canResize, active: this.resizeHandler, visibleProps: visibleProps, providers: this.providers, groups: this.groups, dimensionCol: this.dimensionCol.state, depth: this.groupingDepth, onResize: (changedX, startIndex, endIndex) => this.onResizeGroup(changedX, startIndex, endIndex), additionalData: this.additionalData })),
      index$1.h("div", { class: `${index.HEADER_ROW_CLASS} ${index.HEADER_ACTUAL_ROW_CLASS}` }, cells),
    ];
  }
  get providers() {
    return {
...
function throttle(func, wait, options) {
  ...
  if (debounce.isObject_1(options)) {
    leading = 'leading' in options ? !!options.leading : leading;
    trailing = 'trailing' in options ? !!options.trailing : trailing;
  }
  return debounce.debounce_1(func, wait, {
    'leading': leading,
    'maxWait': wait,
    'trailing': trailing
  });
...
async function resizeObserver() {
  if (!('ResizeObserver' in window)) {
    const module = await Promise.resolve().then(function () { return require('./resize-observer-d8b0ae52.js'); });
...
const RevogrViewportScroll = class {
  constructor(hostRef) {
    index$1.registerInstance(this, hostRef);
    this.scrollViewport = index$1.createEvent(this, "scrollViewport", 7);
    this.resizeViewport = index$1.createEvent(this, "resizeViewport", 7);
    this.scrollchange = index$1.createEvent(this, "scrollchange", 7);
    this.silentScroll = index$1.createEvent(this, "silentScroll", 7);
  ...
  render() {
    return (index$1.h(index$1.Host, { onWheel: this.horizontalMouseWheel, onScroll: (e) => this.onScroll('rgCol', e) }, index$1.h("div", { class: "inner-content-table", style: { width: `${this.contentWidth}px` } }, index$1.h("div", { class: "header-wrapper", ref: e => (this.header = e) }, index$1.h("slot", { name: viewport_helpers.HEADER_SLOT })), index$1.h("div", { class: "vertical-inner", ref: el => (this.verticalScroll = el), onWheel: this.verticalMouseWheel, onScroll: (e) => this.onScroll('rgRow', e) }, index$1.h("div", { class: "content-wrapper", style: { height: `${this.contentHeight}px` } }, index$1.h("slot", { name: viewport_helpers.CONTENT_SLOT }))), index$1.h("div", { class: "footer-wrapper", ref: e => (this.footer = e) }, index$1.h("slot", { name: viewport_helpers.FOOTER_SLOT })))));
  }
  /**
  ...
  get horizontalScroll() { return index$1.getElement(this); }
};
RevogrViewportScroll.style = revogrViewportScrollStyleCss;

exports.revogr_cell = RevogridCellRenderer;
...
//# sourceMappingURL=revogr-cell_4.cjs.entry.js.map