...
class ColumnService {
  get columns() {
    return index.getVisibleSourceItem(this.source);
  }
  ...
  checkGrouping(cols) {
    for (let rgCol of cols) {
      if (viewport_helpers.isGroupingColumn(rgCol)) {
  ...
  mergeProperties(r, c, defaultProps) {
    var _a;
    const cellClass = {
      [index.CELL_CLASS]: true,
      [index.DISABLED_CLASS]: this.isReadOnly(r, c),
    };
  ...
  getRowClass(r, prop) {
    const model = index.getSourceItem(this.dataStore, r) || {};
    return model[prop] || '';
  }
  getCellData(r, c) {
    const data = this.rowDataModel(r, c);
  ...
  rowDataModel(rowIndex, colIndex) {
    ...
    const model = index.getSourceItem(this.dataStore, rowIndex) || {};
  ...
  getRangeData(d, columns) {
    ...
    for (let rowIndex = d.newRange.y, i = 0; rowIndex < d.newRange.y1 + 1; rowIndex++, i++) {
      ...
      const copyRow = index.getSourceItem(this.dataStore, oldRowIndex) || {};
      // columns
  ...
  getTransformedDataToApply(start, data) {
    ...
    const range = viewport_helpers.getRange(start, {
      y: rowIndex - 1,
      x: maxCol,
    });
    return {
  ...
  applyRangeData(data) {
    ...
    for (let rowIndex in data) {
      const oldModel = (items[rowIndex] = index.getSourceItem(this.dataStore, parseInt(rowIndex, 10)));
      if (!oldModel) {
        continue;
      }
    ...
    index.setSourceByVirtualIndex(this.dataStore, items);
  }
    ...
    for (let rowIndex = d.y, i = 0; rowIndex < d.y1 + 1; rowIndex++, i++) {
      ...
      for (let colIndex = d.x, j = 0; colIndex < d.x1 + 1; colIndex++, j++) {
        ...
        area.push({
          ...
          rowIndex,
          colIndex,
          model: index.getSourceItem(store, rowIndex),
          type,
          colType: this.type,
        });
      }
  ...
  copyRangeArray(range, store) {
    ...
    const props = viewport_helpers.lodash.slice(cols, range.x, range.x1 + 1).map(v => v.prop);
    const toCopy = [];
    const mapping = {};
    // rows indexes
    for (let i = range.y; i <= range.y1; i++) {
      ...
      for (let prop of props) {
        const item = index.getSourceItem(store, i);
        // if no item - skip
        if (!item) {
          continue;
        }
...
class LocalScrollService {
  ...
  convert(pos, param, toReal = true) {
    ...
    if (toReal) {
      return index.scaleValue(pos, from, to);
    }
    return index.scaleValue(pos, to, from);
  }
}
...
//# sourceMappingURL=localScrollService-948e38e3.js.map