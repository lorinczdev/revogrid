...
function getCurrentCell({ x, y }, { el, rows, cols }) {
  ...
  const rgRow = index.getItemByPosition(rows, cellY);
  const rgCol = index.getItemByPosition(cols, cellX);
  // before first
...
function getCell({ x, y, x1, y1 }, dimensionRow, dimensionCol) {
  const top = index.getItemByIndex(dimensionRow, y).start;
  const left = index.getItemByIndex(dimensionCol, x).start;
  const bottom = index.getItemByIndex(dimensionRow, y1).end;
  const right = index.getItemByIndex(dimensionCol, x1).end;
  return {
    left,
    right,
    top,
...
const RevogrFocus$1 = class {
  constructor(hostRef) {
    ...
    this.beforeFocusRender = index$1.createEvent(this, "before-focus-render", 7);
    this.beforeScrollIntoView = index$1.createEvent(this, "beforescrollintoview", 7);
  ...
  changed(e, focus) {
    ...
    const model = index.getSourceItem(this.dataStore, focus.y);
    const column = index.getSourceItem(this.colData, focus.x);
  ...
  render() {
    ...
    if (data) {
      ...
      const extra = this.focusTemplate && this.focusTemplate(index$1.h, detail);
      return index$1.h(index$1.Host, { class: index.FOCUS_CLASS, style: style }, index$1.h("slot", null), extra);
    }
  }
...
class SelectionStoreService {
  ...
  focus(cell, isMulti = false) {
    ...
    if (isMulti) {
      ...
      if (start) {
        return this.config.changeRange(viewport_helpers.getRange(start, end));
      }
    }
    // single focus
...
function isEditInput(el) {
  return !!(el === null || el === void 0 ? void 0 : el.closest(`.${index.EDIT_INPUT_WR}`));
}

const DIRECTION_CODES = [
  keyCodes_utils.codesLetter.TAB,
  keyCodes_utils.codesLetter.ARROW_UP,
  keyCodes_utils.codesLetter.ARROW_DOWN,
  keyCodes_utils.codesLetter.ARROW_LEFT,
  keyCodes_utils.codesLetter.ARROW_RIGHT,
];
class KeyboardService {
  ...
  async keyDown(e, canRange) {
    ...
    if (this.sv.selectionStoreService.edited) {
      switch (e.code) {
        case keyCodes_utils.codesLetter.ESCAPE:
          this.sv.cancelEdit();
          break;
      }
      return;
    ...
    // IF NOT EDIT MODE
    // pressed clear key
    if (this.sv.selectionStoreService.ranged && keyCodes_utils.isClear(e.code)) {
      this.sv.clearCell();
      return;
    }
    ...
    if (keyCodes_utils.codesLetter.TAB === e.code) {
      this.keyChangeSelection(e, canRange);
      return;
    }
    // pressed enter
    if (keyCodes_utils.isEnterKey(e.code)) {
      this.sv.applyEdit();
      return;
    }
    // copy operation
    if (keyCodes_utils.isCopy(e)) {
      return;
    }
    // cut operation
    if (keyCodes_utils.isCut(e)) {
      return;
    }
    // paste operation
    if (keyCodes_utils.isPaste(e)) {
      this.sv.internalPaste();
      return;
    }
    // select all
    if (keyCodes_utils.isAll(e)) {
      if (canRange) {
        this.selectAll(e);
      }
      return;
    ...
    // pressed letter key
    if (keyCodes_utils.isLetterKey(e.keyCode)) {
  ...
  async keyChangeSelection(e, canRange) {
    ...
    await index.timeout(index.RESIZE_INTERVAL + 30);
    const range = this.sv.selectionStore.get('range');
    const focus = this.sv.selectionStore.get('focus');
  ...
  keyPositionChange(changes, range, focus, isMulti = false) {
    ...
    if (isMulti) {
      ...
      const range = viewport_helpers.getRange(data.start, data.end);
  ...
  changeDirectionKey(e, canRange) {
    ...
    switch (e.code) {
      case keyCodes_utils.codesLetter.ARROW_UP:
        return { changes: { y: -1 }, isMulti };
      case keyCodes_utils.codesLetter.ARROW_DOWN:
        return { changes: { y: 1 }, isMulti };
      case keyCodes_utils.codesLetter.ARROW_LEFT:
        return { changes: { x: -1 }, isMulti };
      case keyCodes_utils.codesLetter.TAB:
      case keyCodes_utils.codesLetter.ARROW_RIGHT:
        return { changes: { x: 1 }, isMulti };
    }
  }
}
...
class AutoFillService {
  ...
  renderAutofill(range, selectionFocus) {
    ...
    return (index$1.h("div", { class: {
        [index.CELL_HANDLER_CLASS]: true,
        [index.MOBILE_CLASS]: true,
  ...
  selectionMouseMove(e) {
    ...
    if (!this.onMouseMoveAutofill) {
      this.onMouseMoveAutofill = debounce.debounce_1((e, data) => this.doAutofillMouseMove(e, data), 5);
    }
    if (this.isAutoFill) {
  ...
  doAutofillMouseMove(event, data) {
    ...
    const x = events.getFromEvent(event, 'clientX', index.MOBILE_CLASS);
    const y = events.getFromEvent(event, 'clientY', index.MOBILE_CLASS);
    // skip touch
    if (x === null || y === null) {
      return;
    }
    ...
    if (isSame) {
      ...
      this.sv.setTempRange({
        area: viewport_helpers.getRange(this.autoFillInitial, this.autoFillLast),
  ...
  clearAutoFillSelection() {
    ...
    if (this.autoFillInitial) {
      ...
      if (this.autoFillType === AutoFillType.autoFill) {
        const range = viewport_helpers.getRange(this.autoFillInitial, this.autoFillLast);
        if (range) {
  ...
  onRangeApply(data, range) {
    ...
    for (let rowIndex in data) {
      models[rowIndex] = index.getSourceItem(this.sv.dataStore, parseInt(rowIndex, 10));
    }
  ...
  applyRangeOnly(start, end) {
    ...
    const newRange = viewport_helpers.getRange(start, end);
    this.sv.setRange(newRange);
  }
}
...
class ClipboardService {
  ...
  renderClipboard(readonly = false) {
    return index$1.h("revogr-clipboard", { readonly: readonly, onCopyRegion: e => this.onCopy(e.detail), onClearRegion: () => this.sv.rangeClear(), ref: e => (this.clipboard = e), onPasteRegion: e => this.onPaste(e.detail) });
  }
  getRegion() {
    const focus = this.sv.selectionStoreService.focused;
    let range = this.sv.selectionStoreService.ranged;
    if (!range) {
      range = viewport_helpers.getRange(focus, focus);
    }
    return range;
  }
  onCopy(e) {
...
const OverlaySelection = class {
  constructor(hostRef) {
    index$1.registerInstance(this, hostRef);
    this.internalCopy = index$1.createEvent(this, "internalCopy", 7);
    this.internalPaste = index$1.createEvent(this, "internalPaste", 7);
    this.internalCellEdit = index$1.createEvent(this, "internalCellEdit", 7);
    this.beforeFocusCell = index$1.createEvent(this, "beforeFocusCell", 7);
    this.setEdit = index$1.createEvent(this, "setEdit", 7);
    this.beforeApplyRange = index$1.createEvent(this, "before-apply-range", 7);
    this.beforeSetRange = index$1.createEvent(this, "before-set-range", 7);
    this.beforeEditRender = index$1.createEvent(this, "before-edit-render", 7);
    this.setRange = index$1.createEvent(this, "setRange", 7);
    this.selectAll = index$1.createEvent(this, "selectall", 7);
    this.cancelEdit = index$1.createEvent(this, "cancelEdit", 7);
    this.setTempRange = index$1.createEvent(this, "setTempRange", 7);
    this.applyFocus = index$1.createEvent(this, "applyFocus", 7);
    this.focusCell = index$1.createEvent(this, "focusCell", 7);
    this.beforeRangeDataApply = index$1.createEvent(this, "beforeRangeDataApply", 7);
    this.internalSelectionChanged = index$1.createEvent(this, "internalSelectionChanged", 7);
    this.beforeRangeCopyApply = index$1.createEvent(this, "beforeRangeCopyApply", 7);
    this.internalRangeDataApply = index$1.createEvent(this, "internalRangeDataApply", 7);
    this.rangeClipboardCopy = index$1.createEvent(this, "rangeClipboardCopy", 7);
    this.rangeClipboardPaste = index$1.createEvent(this, "rangeClipboardPaste", 7);
    this.beforeKeyDown = index$1.createEvent(this, "beforekeydown", 7);
    this.beforeKeyUp = index$1.createEvent(this, "beforekeyup", 7);
    this.beforeCellSave = index$1.createEvent(this, "before-cell-save", 7);
  ...
  renderRange(range) {
    ...
    return [
      index$1.h("div", { class: index.SELECTION_BORDER_CLASS, style: style }, this.isMobileDevice && index$1.h("div", { class: "range-handlers" }, index$1.h("span", { class: index.MOBILE_CLASS }), index$1.h("span", { class: index.MOBILE_CLASS })))
    ];
  }
  renderEditCell() {
    // if can edit
    ...
    const style = getElStyle(range, this.dimensionRow.state, this.dimensionCol.state);
    return (index$1.h("revogr-edit", { ref: el => {
        this.revogrEdit = el;
      }, onCellEdit: e => {
  ...
  render() {
    ...
    if (editCell) {
      ...
      if (this.canDrag) {
        els.push(index$1.h("revogr-order-editor", { ref: e => (this.orderEditor = e), dataStore: this.dataStore, dimensionRow: this.dimensionRow, dimensionCol: this.dimensionCol, parent: this.element, onInternalRowDragStart: e => this.onRowDragStart(e) }));
      }
    }
    return (index$1.h(index$1.Host, { class: { mobile: this.isMobileDevice },
      ...
      }, onMouseDown: (e) => this.onElementMouseDown(e), onTouchStart: (e) => this.onElementMouseDown(e, true) }, els, index$1.h("slot", { name: "data" })));
  }
  ...
  onElementMouseDown(e, touch = false) {
    ...
    if (this.range) {
      ...
      else if (events.verifyTouchTarget(e.touches[0], index.MOBILE_CLASS)) {
        e.preventDefault();
      }
    }
  }
  ...
  async focusNext() {
    const canFocus = await this.keyboardService.keyChangeSelection(new KeyboardEvent('keydown', {
      code: keyCodes_utils.codesLetter.ARROW_DOWN,
    }), this.range);
    if (!canFocus) {
      this.closeEdit();
    }
  ...
  clearCell() {
    if (this.selectionStoreService.ranged && !viewport_helpers.isRangeSingleCell(this.selectionStoreService.ranged)) {
...
const RevogrRowHeaders = class {
  constructor(hostRef) {
    index$1.registerInstance(this, hostRef);
    this.scrollViewport = index$1.createEvent(this, "scrollViewport", 3);
    this.elementToScroll = index$1.createEvent(this, "elementToScroll", 3);
  ...
  render() {
    ...
    for (let data of this.dataPorts) {
      ...
      // initiate row data
      const dataStore = new index.DataStore(data.type);
      dataStore.updateData(data.dataStore.get('source'));
      // initiate column data
      const colData = new index.DataStore('colPinStart');
      ...
      dataViews.push(index$1.h("revogr-data", Object.assign({}, viewData)));
    ...
    const viewportHeader = Object.assign(Object.assign({}, this.headerProp), { colData: typeof this.rowHeaderColumn === 'object' ? [this.rowHeaderColumn] : [], viewportCol: viewport.store, canResize: false, type: index.ROW_HEADER_TYPE, parent, slot: viewport_helpers.HEADER_SLOT });
    return (index$1.h(index$1.Host, { class: { [index.ROW_HEADER_TYPE]: true }, key: index.ROW_HEADER_TYPE }, index$1.h("revogr-viewport-scroll", Object.assign({}, viewportScroll, { "row-header": true }), index$1.h("revogr-header", Object.assign({}, viewportHeader)), dataViews)));
  }
};
...
const RevogrScrollVirtual = class {
  constructor(hostRef) {
    index$1.registerInstance(this, hostRef);
    this.scrollVirtual = index$1.createEvent(this, "scrollVirtual", 7);
    this.scrollSize = 0;
  ...
  render() {
    ...
    return (index$1.h(index$1.Host, Object.assign({}, { 'auto-hide': this.isAutoHide }, { onScroll: (e) => this.onScroll(e) }), index$1.h("div", { style: { [sizeType]: `${this.extContentSize(this.viewportStore.get('virtualSize'), this.dimensionStore.get('realSize'))}px` } })));
  }
  extContentSize(vsize, contentSize) {
    return localScrollService.LocalScrollService.getVirtualContentSize(contentSize, this.size, vsize);
  }
  get element() { return index$1.getElement(this); }
};
...
const RevogrFocus = class {
  constructor(hostRef) {
    index$1.registerInstance(this, hostRef);
    this.onChange = viewport_helpers.lodash.throttle((e) => this.doChange(e), 300);
    this.selectionStore = undefined;
    this.dimensionRow = undefined;
    this.dimensionCol = undefined;
  }
  ...
  render() {
    ...
    return (index$1.h(index$1.Host, { class: {
        [index.TMP_SELECTION_BG_CLASS]: true,
        [type || '']: true,
      }, style: style, hidden: false }, index$1.h("div", { class: directionClass, ref: (e) => (this.el = e) })));
  }
  getRange() {
...
//# sourceMappingURL=revogr-focus_5.cjs.entry.js.map